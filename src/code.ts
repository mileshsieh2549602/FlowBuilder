...
async function insertDefaultNodeOnConnector(connector: GroupNode): Promise<void> {
  ...
  const startBounds = getNodeBoundsInPage(startFrame);
  const endBounds = getNodeBoundsInPage(endFrame);
  const direction: HorizontalMagnet =
    endBounds.x + endBounds.width / 2 >= startBounds.x + startBounds.width / 2 ? "RIGHT" : "LEFT";

  const startPoint = getAttachPoint(startFrame, direction === "RIGHT" ? "RIGHT" : "LEFT");
  const endPoint = getAttachPoint(endFrame, direction === "RIGHT" ? "LEFT" : "RIGHT");
